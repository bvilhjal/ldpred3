...
All notable changes to **pyLDpred2** are recorded here. The format is loosely
...
- `run_ldpred2_prs` now warns that `dentist=True` is ignored on the `ld_cache`
  path (cached LD is authoritative). (#30)

## [0.1.0]

First public version: a dependency-light (NumPy-only, optional Numba) LDpred2
with a full summary-statistics → polygenic-score pipeline, no R required.
...
- **Streaming genome-wide LDpred2-auto inference** of h², polygenicity and
  predictive r² with no validation cohort, plus real-world sumstats tests. (#28, #27)
- **Bivariate (two-trait) LDpred2** and **LD Score regression** for h² and
  genetic correlation (`ldpred2_auto_bivariate`, `ldsc_h2`, `ldsc_rg`). (#24)
...
[Unreleased]: https://github.com/bvilhjal/iprs/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/bvilhjal/iprs/releases/tag/v0.1.0
