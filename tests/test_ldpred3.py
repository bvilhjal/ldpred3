"""Synthetic-data tests for the basic LDpred3 implementation.

The tests simulate a single LD block with a known sparse genetic architecture,
generate GWAS marginal effects from the LDpred3 model
...
Run with ``pytest`` or directly with ``python tests/test_ldpred3.py``.
"""
...
import ldpred3.ldpred3 as ld_core
from ldpred3.ldpred3 import (  # noqa: E402
    block_diagonal_ld,
    ldpred3_auto,
    ldpred3_by_blocks,
    ldpred3_grid,
...
def test_inf_beats_marginal():
    ...
    beta_inf = ldpred3_inf(corr, beta_hat, n, h2=0.5)
...
def test_grid_beats_marginal():
    ...
    beta_grid = ldpred3_grid(corr, beta_hat, n, h2=0.5, p=0.05,
...
def test_auto_recovers_hyperparams():
    ...
    res = ldpred3_auto(corr, beta_hat, n, h2_init=0.3, p_init=0.1,
...
def test_sparse_matches_dense_when_full():
    ...
    dense = ldpred3_grid(corr, beta_std, n_vec, h2=0.5, p=0.05,
                         burn_in=40, num_iter=120, seed=3)
    full = sparsify_ld(corr, threshold=0.0)        # keep all entries
    sp = ldpred3_grid(full, beta_std, n_vec, h2=0.5, p=0.05,
                      burn_in=40, num_iter=120, seed=3)
...
def test_sparse_inf_matches_dense_when_full():
    ...
    n_vec = np.full(corr.shape[0], float(n))
    dense = ldpred3_inf(corr, beta_hat, n_vec, h2=0.5)
    sp = ldpred3_inf(sparsify_ld(corr, threshold=0.0), beta_hat, n_vec, h2=0.5)
...
def test_banded_sparse_recovers_signal():
    ...
    beta_grid = ldpred3_grid(sp, beta_hat, n_vec, h2=0.5, p=0.05,
...
def test_warm_start_recovers_signal():
    ...
    n_vec = np.full(corr.shape[0], float(n))
    cold = ldpred3_grid(corr, beta_hat, n_vec, h2=0.5, p=0.05,
                        burn_in=100, num_iter=300, seed=1)
    warm = ldpred3_grid(corr, beta_hat, n_vec, h2=0.5, p=0.05,
...
def test_adaptive_stopping_stops_early():
    ...
    n_vec = np.full(corr.shape[0], float(n))
    res = ldpred3_auto(corr, beta_hat, n_vec, h2_init=0.3, p_init=0.1,
...
def test_block_diagonal_inf_matches_per_block():
    ...
    m = len(bhat)
    per_block = np.concatenate([ldpred3_inf(c, bhat[idx], n[idx], h2=0.5 * len(idx) / m)
                                for c, idx in blocks])
    glob = ldpred3_inf(block_diagonal_ld(blocks), bhat, n, h2=0.5)
    assert np.allclose(per_block, glob, atol=1e-4)


def test_global_auto_recovers_signal():
    """LDpred3-auto with global hyper-parameters recovers the signal."""
    blocks, bhat, true_beta, n = _make_blocks(nb=5, seed=2)
    beta = ldpred3_by_blocks(blocks, bhat, n, method="auto",
...
def test_parallel_auto_recovers_signal():
    ...
    beta = ldpred3_by_blocks(blocks, bhat, n, method="auto", burn_in=40,
...
def test_hyperparameter_validation():
    ...
    with pytest.raises(ValueError):
        ldpred3_inf(corr, beta_hat, nv, h2=0.0)
    with pytest.raises(ValueError):
        ldpred3_grid(corr, beta_hat, nv, h2=-1.0, p=0.05)
    with pytest.raises(ValueError):
        ldpred3_grid(corr, beta_hat, nv, h2=0.5, p=1.5)
    with pytest.raises(ValueError):
        ldpred3_auto(corr, beta_hat, nv, p_init=0.0)
    with pytest.raises(ValueError):
        ldpred3_auto(corr, beta_hat, nv, h2_init=-0.1)
    with pytest.raises(ValueError):
        ldpred3_auto(corr, beta_hat, nv, h2_bounds=(0.5, 0.1))
    with pytest.raises(ValueError):       # non-positive n_eff
        ldpred3_inf(corr, beta_hat, np.zeros(50), h2=0.5)
...
def test_global_auto_rejects_bad_blocks():
    ...
    with pytest.raises(ValueError):
        ldpred3_by_blocks(bad, bhat, n, method="auto", global_hyper=True,
...
def test_inf_zero_beta_is_finite():
    ...
    zero = np.zeros(80)
    dense = ldpred3_inf(corr, zero, nv, h2=0.5)
    sparse = ldpred3_inf(sparsify_ld(corr, threshold=0.0), zero, nv, h2=0.5)
...
def test_extreme_params_no_overflow():
    ...
    nv = np.full(60, 1e7)
    out = ldpred3_grid(corr, beta_hat * 50, nv, h2=0.9, p=1e-6,
...
if __name__ == "__main__":
    ...
    base = _corr(beta_hat, true_beta)
    inf = _corr(ldpred3_inf(corr, beta_hat, n, h2=0.5), true_beta)
    grid = _corr(
        ldpred3_grid(corr, beta_hat, n, h2=0.5, p=0.05, seed=1), true_beta
    )
    res = ldpred3_auto(corr, beta_hat, n, seed=1)
    auto = _corr(res.beta_est, true_beta)

    print("Correlation with true effects (higher is better):")
    print(f"  marginal beta_hat : {base:.3f}")
    print(f"  LDpred3-inf       : {inf:.3f}")
