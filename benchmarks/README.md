...
| `bench_ldpred3_scaling.py` | LDpred3 fit time + LD memory vs **SNP density** on realistic recombination-limited LD (~1,700 blocks, densified via the **mutation-rate lever** so blocks grow, not multiply). Reports dense/banded/low-rank/streaming memory (dense ~quadratic → 15 GB wall ~2.4M; low-rank ~m^1.25 → ~8M; streaming flat) and inf/grid/auto time (inf's O(k³) overtakes grid/auto by ~1.2M); → `ldpred3_scaling.{csv,png}` | ✓ |
